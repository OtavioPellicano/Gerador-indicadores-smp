    ...
    ui(new Ui::DetalheIndicador)
{
    ui->setupUi(this);
    QStringList header;
    header << "Prestadora" << "UF" << "Conexão" << "Medições Válidas" << "Coletores Válidos" <<
              "Índice 39" << "Índice 40" << "Índice 41" << "Índice 42";
    ui->tableWidget_detalhe->setColumnCount(9);
    ...
    for(auto itQStr = vecIndicadores.begin() + 1; itQStr != vecIndicadores.end(); ++itQStr)
    {
        ...
        if(strCsv[colPrest] == prestadora)
            if(strCsv[colUF] == uf)
            {
                ...
                ui->tableWidget_detalhe->setItem(linha, 4, new QTableWidgetItem(strCsv[colColVal]));
                ui->tableWidget_detalhe->setItem(linha, 5, new QTableWidgetItem(strCsv[colInd_39]));
                ui->tableWidget_detalhe->setItem(linha, 6, new QTableWidgetItem(strCsv[colInd_40]));
                ui->tableWidget_detalhe->setItem(linha, 7, new QTableWidgetItem(strCsv[colInd_41]));
                ui->tableWidget_detalhe->setItem(linha, 8, new QTableWidgetItem(strCsv[colInd_42]));
